...
- 🎯 Entry points to execute your program using `python -m <fast_api_tmpl>` or `$ fast_api_tmpl` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/sarath-ps/fast-api-tmpl/branch/main/graph/badge.svg?token=fast-api-tmpl_token_here)](https://codecov.io/gh/sarath-ps/fast-api-tmpl)
[![CI](https://github.com/sarath-ps/fast-api-tmpl/actions/workflows/main.yml/badge.svg)](https://github.com/sarath-ps/fast-api-tmpl/actions/workflows/main.yml)

Awesome fast_api_tmpl created by sarath-ps

## Install

from source
```bash
git clone https://github.com/sarath-ps/fast-api-tmpl fast_api_tmpl
cd fast_api_tmpl
make install
```
...
```bash
pip install fast_api_tmpl
```

## Executing

```bash
$ fast_api_tmpl run --port 8080
```

or

```bash
python -m fast_api_tmpl run --port 8080
```

or

```bash
$ uvicorn fast_api_tmpl:app
```
...
```bash
❯ fast_api_tmpl --help
Usage: fast_api_tmpl [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fast_api_tmpl create-user --help
Usage: fast_api_tmpl create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fast_api_tmpl create-user admin admin --superuser
```
...
Run with `fast_api_tmpl run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fast_api_tmpl create-user`.
...
-----------------------------------------------------
fast_api_tmpl/__init__.py              4      0   100%
fast_api_tmpl/app.py                  16      1    94%
fast_api_tmpl/cli.py                  21      0   100%
fast_api_tmpl/config.py                5      0   100%
fast_api_tmpl/db.py                   10      0   100%
fast_api_tmpl/models/__init__.py       0      0   100%
fast_api_tmpl/models/content.py       47      1    98%
fast_api_tmpl/routes/__init__.py      11      0   100%
fast_api_tmpl/routes/content.py       52     25    52%
fast_api_tmpl/routes/security.py      15      1    93%
fast_api_tmpl/routes/user.py          52     26    50%
fast_api_tmpl/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from fast_api_tmpl.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fast_api_tmpl/default.toml.

### As environment variables
```bash
export fast_api_tmpl_KEY=value
export fast_api_tmpl_KEY="@int 42"
export fast_api_tmpl_KEY="@jinja {{ this.db.uri }}"
export fast_api_tmpl_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fast_api_tmpl_ENV=production fast_api_tmpl run
```
